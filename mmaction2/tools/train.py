...
def merge_args(cfg, args):
    ...
    # if args.amp:
    #     # 只有在 GPU/NPU/MLU/MUSA 可用时才启用 AMP
    #     from mmengine.device import is_cuda_available, is_npu_available, is_mlu_available
    #     if not (is_cuda_available() or is_npu_available() or is_mlu_available()):
    #         import warnings
    #         warnings.warn(
    #             '当前环境不支持硬件加速（GPU/NPU/MLU），已自动禁用 AMP，'
    #             '训练将使用普通精度。'
    #             )
    #     else:
    #         cfg.optim_wrapper.type = 'AmpOptimizerWrapper'
    #         cfg.optim_wrapper.setdefault('loss_scale', 'dynamic')
